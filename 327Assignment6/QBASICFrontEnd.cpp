...
bool validateAccountCA(string account, vector<string> acctList)
{
	...
	if (!isNum(account))
	{
		printf("Invalid Account Number. Cannot contain letters or symbols. ");
		return false;
	}
...
bool validateMoney(string amount, bool isMachine)
{
	...
	if (!isNum(amount))
	{
		printf("Invalid Amount. Cannot contain letters or symbols. ");
		return false;
	}
	...
	if (!isAgent)
	{
		...
		for (i = 0; i < transSum.size(); i++)
		{
			...
			if (transSum[i].substr(0, 3) == "WDR")
			{
				...
				if (transSum[i].substr(4, 7) == inputNum)
				{
...
int main(int argc, char* argv[])
{
	...
	while(inStr != "exit")
	{
		...
		//report successful and see if the user wants to login again
		//entering exit will end outer loop, entering login will send user to enter mode selection
		//entering anything else will send user into login loop
