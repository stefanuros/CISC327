...
vector<string> upDEP(vector<string> accounts, string transaction, int acctPos)
{
	...
	//If balance greater than 8 digits
	if(newAmt > 99999999)
	{
		return accounts;
	}

	//If adding numbers makes the number of characters less than 3
	//Making amt at least 3 digits
	if(newAmt < 10)
	{
		accounts[acctPos] = accounts[acctPos].substr(0, 7) + " 00" + to_string(newAmt) + " " + findAcctName(accounts[acctPos], 8);
	}
	else if(newAmt < 100)
	{
		accounts[acctPos] = accounts[acctPos].substr(0, 7) + " 0" + to_string(newAmt) + " " + findAcctName(accounts[acctPos], 8);
	}
	else
	{
...
vector<string> upWDR_XFR(vector<string> accounts, string transaction, int acctPos)
{
	...
	if(newAmt < 10)
	{
		accounts[acctPos] = oldAcct.substr(0, 7) + " 00" + to_string(newAmt) + " " + findAcctName(oldAcct, 8);
	}
	else if(newAmt < 100)
	{
		accounts[acctPos] = oldAcct.substr(0, 7) + " 0" + to_string(newAmt) + " " + findAcctName(oldAcct, 8);
	}
	else
	{
